...
import {
    ...
    DELETE_USER_SUCCESS,
    DELETE_USER_FAIL,
    UPDATE_USERCART_REQUEST,
    UPDATE_USERCART_SUCCESS,
    UPDATE_USERCART_FAIL,
    LOGOUT_SUCCESS,
    LOGOUT_FAIL,
    CLEAR_ERRORS
 ...
 * Update User Cart
 */
export const updateUserCart = (userData) => async (dispatch) => {
    try {
        dispatch({ type: UPDATE_USERCART_REQUEST });
        ...
        const { data } = await axios.put(`${API_URL}/api/v1/me/update/cart`, userData, config);

        dispatch({ 
            type: UPDATE_USERCART_SUCCESS, 
            payload: data.success
        });

    } catch (error) {
        dispatch({ 
            type: UPDATE_USERCART_FAIL, 
            payload: error.response.data.message
        });
    }
}
