...
export const getProducts = (keyword = '', currentPage = 1, price, category, ratings = 0) => async (dispatch) => {
    try {
        ...
        let link = `${API_URL}/api/v1/products?keyword=${keyword}&page=${currentPage}&price[lte]=${price[1]}&price[gte]=${price[0]}&ratings[gte]=${ratings}`;

        // This because category is only filter that isn't specified by default
        if (category) {
            link = `${API_URL}/api/v1/products?keyword=${keyword}&page=${currentPage}&price[lte]=${price[1]}&price[gte]=${price[0]}&category=${category}&ratings[gte]=${ratings}`;
        }
...
export const getAllProducts = () => async (dispatch) => {
    try {
        ...
        let link = `${API_URL}/api/v1/allproducts`;
...
export const deleteReview = (id, productId) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.delete(`${API_URL}/api/v1/reviews?id=${id}&productId=${productId}`)
