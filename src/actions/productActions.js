...
export const getProducts = (keyword = '', currentPage = 1, price, category, ratings = 0) => async (dispatch) => {
    try {
        ...
        let link = `${URL}/api/v1/products?keyword=${keyword}&page=${currentPage}&price[lte]=${price[1]}&price[gte]=${price[0]}&ratings[gte]=${ratings}`;

        // This because category is only filter that isn't specified by default
        if (category) {
            link = `${URL}/api/v1/products?keyword=${keyword}&page=${currentPage}&price[lte]=${price[1]}&price[gte]=${price[0]}&category=${category}&ratings[gte]=${ratings}`;
        }
...
export const getAllProducts = () => async (dispatch) => {
    try {
        ...
        let link = `${URL}/api/v1/allproducts`;
